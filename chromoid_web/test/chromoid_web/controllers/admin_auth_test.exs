defmodule ChromoidWeb.AdminAuthTest do
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, ChromoidWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "log_in_admin/3" do
    ...
    test "writes a cookie if remember_me is configured", %{conn: conn, admin: admin} do
      ...
      assert get_session(conn, :admin_token) == conn.cookies["admin_remember_me"]

      assert %{value: signed_token, max_age: max_age} = conn.resp_cookies["admin_remember_me"]
  ...
  describe "logout_admin/1" do
    test "erases session and cookies", %{conn: conn, admin: admin} do
      ...
      conn =
        ...
        |> put_req_cookie("admin_remember_me", admin_token)
        |> fetch_cookies()
        |> AdminAuth.log_out_admin()

      refute get_session(conn, :admin_token)
      refute conn.cookies["admin_remember_me"]
      assert %{max_age: 0} = conn.resp_cookies["admin_remember_me"]
      assert redirected_to(conn) == "/"
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      ChromoidWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "fetch_current_admin/2" do
    ...
    test "authenticates admin from cookies", %{conn: conn, admin: admin} do
      ...
      admin_token = logged_in_conn.cookies["admin_remember_me"]
      %{value: signed_token} = logged_in_conn.resp_cookies["admin_remember_me"]

      conn =
        conn
        |> put_req_cookie("admin_remember_me", signed_token)
