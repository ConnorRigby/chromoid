defmodule SpeediViewUI.Scene.Dash do
  @moduledoc """
  ...
  alias SpeediViewUI.Layout
  import SpeediViewUI.Component.Gauge, only: [gauge: 3]
  import SpeediViewUI.Component.StatusBar, only: [status_bar: 3]
  import SpeediViewUI.Util
  ...
  def handle_info(_info, state) do
    IO.puts "building new graph"
    graph =
      Graph.build()
      |> rect({state.vp_width, state.vp_height}, id: :background, fill: {51, 51, 51}, stroke: {2, :white})
      |> gauge(%{data: 0.0, text: float(0.0)}, id: :rpm, translate: state.layout[:rpm] || {0, 50})
      |> gauge(%{data: 0.0, text: float(0.0)}, id: :map, translate: state.layout[:map] || {300, 50})
      |> gauge(%{data: 0.0, text: float(0.0)}, id: :afr, translate: state.layout[:afr] || {600, 50})
      |> gauge(%{data: 0.0, text: float(0.0)}, id: :clt, translate: state.layout[:clt] || {0, 200})
      |> gauge(%{data: 0.0, text: float(0.0)}, id: :tps, translate: state.layout[:tps] || {300, 200})
      |> gauge(%{data: 0.0, text: float(0.0)}, id: :adv, translate: state.layout[:adv] || {600, 200})
      |> status_bar(%{}, id: :status_bar, translate: {0, 0})
