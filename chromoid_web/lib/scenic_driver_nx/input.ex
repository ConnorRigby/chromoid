...
#
defmodule Scenic.Driver.Nerves.NX.Input do
  @moduledoc false
  use Bitwise

  alias Scenic.Driver.Nerves.NX.Cache
  alias Scenic.Driver.Nerves.NX.Font
  ...
  def handle_port_message(<<id::unsigned-integer-size(32)-native, bin::binary>>, state) do
    IO.puts("Unhandled port messages id: #{inspect(id, base: :hex)}, msg: #{inspect(bin)}")
    {:noreply, state}
  end
  ...
  # http://www.NX.org/docs/latest/group__keys.html
  ...
  defp key_to_name(key) do
    raise "Driver.NX recieved unknown input key value: #{inspect(key)}"
  end

  # --------------------------------------------------------
  # defined to follow the Mac modifier keys
  # http://www.NX.org/docs/latest/group__mods.html
  ...
  #    raise "Driver.NX recieved unknown mods: #{inspect(mods)}"
  #  end
