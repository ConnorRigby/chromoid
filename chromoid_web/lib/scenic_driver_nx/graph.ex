...
# and complicated
#
defmodule Scenic.Driver.Nerves.NX.Graph do
  @moduledoc false
  alias Scenic.Driver.Nerves.NX, as: Driver
  alias Scenic.Driver.Nerves.NX.Port
  ...
  def handle_cast(
        :update_clear_color,
        %{socket: socket, master_ref: master_graph_key, clear_color: old_clear_color} = state
      ) do
    with {:ok, master_graph} <- ViewPort.Tables.get_graph(master_graph_key),
      ...
      if clear_color != old_clear_color do
        ...
        Port.clear_color(socket, clear_color)
  ...
  # --------------------------------------------------------
  def handle_cast({:request_input, _input_flags}, %{socket: _socket} = state) do
    #    send_input_flags( input_flags, socket )
    {:noreply, state}
  end
  ...
  defp render_graphs(
    ...
    Task.start_link(fn ->
      ...
      if root_id, do: Port.set_root_dl(socket, root_id)
    end)
  ...
  # --------------------------------------------------------
  # render_one_graph renders one graph out to the C socket.
  ...
  defp render_one_graph(
    ...
    with {:ok, graph} <- ViewPort.Tables.get_graph(graph_key) do
      ...
      _ ->
        ...
        # Send a fake completion event as if it came from the socket.
        msg = <<
          @msg_draw_ready_id::unsigned-integer-size(32)-native,
          dl_id::unsigned-integer-size(32)-native
        >>

        Process.send(driver, {socket, {:data, msg}}, [])
    end
  end
